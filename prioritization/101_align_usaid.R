...
ext(conf) <- ext(rpop)
conf <- mask(conf, rpop)
confs <- extract(conf[[1:2]], cgv, fun = sum, na.rm = TRUE)
confs$ID <- NULL
...
eba$UID <- NULL
eba$level3_ease_of_doing_business_score <- round(eba$level3_ease_of_doing_business_score, 2)

# combine information
# merge all
v <- cbind(cgv, level1, level2, level3)
v$`level3_conflicts per 100,000 rural person` <- round(100000*v$`level3_total_conflict_incidents_2016-21`/v$level1_rural_population)
v$`level3_conflicts_deaths per 100,000 rural person` <- round(100000*v$`level3_total_conflict_deaths_2016-21`/v$level1_rural_population)
...
# save each region as different excel sheet and also add the summary stat
library(tidyverse)
...
dcs1 <- lapply(unique(cgs), function(cg){
  x <- dsf[dsf$cgregion == cg, ]
  x1 <- x %>%
    select(c(farming_system, level1_rural_population:level3_ease_of_doing_business_score)) %>%
    group_by(farming_system) %>% 
    summarise_all(.funs = funs(mean = round(mean(., na.rm = T), 2), std_dev = round(sd(.,na.rm = TRUE), 2))) 
  return(data.frame(cgregion = cg, x1))
})

dcs1 <- bind_rows(dcs1)

dcs2 <- lapply(unique(cgs), function(cg){
  x <- dsf[dsf$cgregion == cg, ]
  x1 <- x %>%
    select(c(NAME_EN, ISO_A3, level1_rural_population:level3_ease_of_doing_business_score)) %>%
...
# dcs3 <- lapply(unique(cgs), function(cg){
#   x <- dsf[dsf$cgregion == cg, ]
#   x1 <- x %>%
#     select(c(NAME_EN, ISO_A3, farming_system, level1_rural_population:level3_ease_of_doing_business_score)) %>%
#     group_by(NAME_EN, ISO_A3, farming_system) %>% 
#     summarise_all(.funs = funs(mean = round(mean(., na.rm = T), 2), std_dev = round(sd(.,na.rm = TRUE), 2))) 
#   return(data.frame(cgregion = cg, x1))
# })
# 
...
tmp <- writexl::write_xlsx(list(SAE = dcg[[1]], LAC = dcg[[2]],
                                ...
                                ESA = dcg[[5]], WCA = dcg[[6]],
                                summary_farming_syetem = dcs1, 
                                summary_country = dcs2, 
